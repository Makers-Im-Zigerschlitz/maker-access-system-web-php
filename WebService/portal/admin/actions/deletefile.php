...
$stmt = $db->prepare("DELETE FROM tblUploads WHERE filename=:fn");
$stmt->bindValue(':fn', filter_input(INPUT_GET, 'filename'), PDO::PARAM_STR);
$stmt->execute();

if ($stmt->rowCount()>0) {
    header("Location: ../index.php?site=documents&message=filedeleted");
